...
export default function AppointmentScheduler() {
  ...
  // ✅ FIX: Default auf 1 setzen (nicht 2) - entspricht DEFAULT_SETTINGS in constants.ts
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // ✅ FIX: Entferne maxBookingsPerSlot hier - wird nur aus Availability API geladen
        // setMaxBookingsPerSlot wird nur in fetchAvailability() aufgerufen
  ...
  const fetchAvailability = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // ✅ FIX: Extrahiere maxBookingsPerSlot aus der Response (EINZIGE Quelle!)
        if (data.maxBookingsPerSlot !== undefined) {
          const maxSlots = Number(data.maxBookingsPerSlot);
          setMaxBookingsPerSlot(maxSlots);
          addDebugLog(`✅ maxBookingsPerSlot from Availability API: ${maxSlots}`);
          ...
          // Fallback wenn API keinen Wert liefert (sollte nicht passieren)
          addDebugLog('⚠️ No maxBookingsPerSlot in API response, keeping default');
