...
import { a as sendAdminNotification, s as sendCustomerNotification } from '../../../chunks/email_BWfHmdDu.mjs';
import { a as getLongLabel } from '../../../chunks/event-config_Bu30ckYK.mjs';
import { D as DEFAULT_SETTINGS } from '../../../chunks/constants_xEU7XdE-.mjs';
...
const PATCH = async ({ request, locals, url }) => {
  ...
  try {
    ...
    try {
      ...
      if (settingsData) {
        const settings2 = JSON.parse(settingsData);
        emailNotifications = settings2.emailNotifications || false;
        adminEmail = settings2.adminEmail || "";
      }
    } catch (error) {
...
async function resetSettings(KV) {
  try {
    await KV.put(SETTINGS_KEY, JSON.stringify(DEFAULT_SETTINGS), { expirationTtl: 60 * 60 * 24 * 90 });
...
async function createGoogleCalendarEvent(appointment, locals) {
  ...
  try {
    ...
    endDate.setMinutes(endDate.getMinutes() + (settings.appointmentDurationMinutes || 30));
    const event = {
