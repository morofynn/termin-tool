...
const GET = async ({ locals }) => {
  try {
    const env = locals?.runtime?.env || Object.assign(__vite_import_meta_env__, { WEBFLOW_API_HOST: "https://api-cdn.webflow.com/v2", WEBFLOW_CMS_SITE_API_TOKEN: process.env.WEBFLOW_CMS_SITE_API_TOKEN, _: process.env._, PATH: process.env.PATH, HOST: process.env.HOST });
    const checks = {
      adminPassword: {
        name: "Admin-Passwort",
        key: "ADMIN_PASSWORD",
      ...
      adminBaseUrl: {
        name: "Admin Base URL",
        key: "ADMIN_BASE_URL",
        status: !!env.ADMIN_BASE_URL,
        value: env.ADMIN_BASE_URL ? `✓ ${env.ADMIN_BASE_URL}` : "✗ Fehlt",
        required: true,
        category: "admin",
        description: "Base URL für Admin-Panel Links in E-Mails (ohne abschließenden /, z.B. https://ihr-projekt.workers.dev)"
      },
      googleCalendarId: {
