...
const DEFAULT_SETTINGS = {
  ...
  // Event End State Defaults
  eventEnded: false,
  eventEndDate: "2025-02-16T18:00:00Z",
  // Default: 16. Februar 2025, 18:00 Uhr
  // Branding Defaults
  companyName: "MORO",
...
function getSettingsChanges(oldSettings, newSettings) {
  ...
  if (oldSettings.eventEnded !== newSettings.eventEnded) {
    changes.push(`Event beendet: ${newSettings.eventEnded ? "Ja" : "Nein"}`);
  }
  if (oldSettings.eventEndDate !== newSettings.eventEndDate) {
    changes.push(`Event-Enddatum: ${new Date(oldSettings.eventEndDate).toLocaleString("de-DE")} → ${new Date(newSettings.eventEndDate).toLocaleString("de-DE")}`);
  }
...
const GET = async ({ locals }) => {
  ...
  try {
    ...
    if (settingsData) {
      ...
      if (settings.eventEnded === void 0) {
        settings.eventEnded = false;
      }
      if (!settings.eventEndDate) {
        settings.eventEndDate = DEFAULT_SETTINGS.eventEndDate;
      }
      const now = /* @__PURE__ */ new Date();
      const eventEnd = new Date(settings.eventEndDate);
      if (now >= eventEnd && !settings.eventEnded) {
        settings.eventEnded = true;
        await KV.put(SETTINGS_KEY, JSON.stringify(settings));
        await createAuditLog(
          KV,
          "Event automatisch beendet",
          `Das Event wurde automatisch auf "beendet" gesetzt, da das Enddatum (${eventEnd.toLocaleString("de-DE")}) erreicht wurde.`,
          void 0,
          "System"
        );
      }
  ...
  try {
    ...
    if (settings.eventEndDate) {
      ...
      if (isNaN(eventEnd.getTime())) {
        return new Response(JSON.stringify({ message: "Ungültiges Event-Enddatum" }), {
          status: 400,
          headers: { "Content-Type": "application/json" }
        });
      }
