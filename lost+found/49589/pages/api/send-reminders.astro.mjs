...
import { e as sendCustomerReminderEmail } from '../../chunks/email_D2sq5GYU.mjs';
    ...
    for (const aptId of appointmentIds) {
      try {
        ...
        if (appointmentDate >= in24Hours && appointmentDate <= in25Hours) {
          ...
          try {
            const emailSent = await sendCustomerReminderEmail(
              appointmentData,
              locals?.runtime?.env
            );
            if (emailSent) {
              ...
              await createAuditLog(
                kv,
                "Erinnerungs-E-Mail",
                `Erinnerung wurde an ${appointment.email} gesendet für Termin am ${appointmentData.date} um ${appointment.time} Uhr.`,
                appointment.id,
                "system"
              );
