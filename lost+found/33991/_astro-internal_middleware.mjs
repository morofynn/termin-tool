...
const onRequest$2 = async (ctx, next) => {
  ...
  if (url.pathname === `${baseUrl}/api/send-reminders`) {
    ...
    const adminPassword2 = locals?.runtime?.env?.ADMIN_PASSWORD || undefined                               || "moro";
  ...
  const adminSecretPath = locals?.runtime?.env?.ADMIN_SECRET_PATH || undefined                                  || "admin";
  const adminPassword = locals?.runtime?.env?.ADMIN_PASSWORD || undefined                               || "moro";
