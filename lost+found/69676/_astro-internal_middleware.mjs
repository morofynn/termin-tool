...
import './chunks/astro-designed-error-pages_BttmyTy8.mjs';
import './chunks/astro/server_CGl8DrLI.mjs';
import { s as sequence } from './chunks/index_nk_oyZrO.mjs';
...
const onRequest$2 = async (ctx, next) => {
  ...
  if (url.pathname === `${baseUrl}/api/send-reminders`) {
    ...
    const adminPassword2 = locals?.runtime?.env?.ADMIN_PASSWORD || "MeinSicheresPasswort123!";
    if (authToken === adminPassword2) {
      return next();
    }
    return new Response(
