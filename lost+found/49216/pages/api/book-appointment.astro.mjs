...
import { v as validateFormData, s as sendAdminNotification, c as sendCustomerConfirmationEmail, d as sendCustomerRequestEmail } from '../../chunks/email_D2sq5GYU.mjs';
...
function toAppointmentData(appointment, appointmentUrl) {
...
Termin verwalten: ${appointmentUrl}
    ...
    try {
      ...
      const appointmentData = toAppointmentData(appointment, appointmentUrl);
      if (adminNotifications && adminEmail) {
        try {
          const action = autoConfirm ? "confirmed" : "requested";
      ...
      try {
        const customerEmailSent = autoConfirm ? await sendCustomerConfirmationEmail(appointmentData, locals?.runtime?.env) : await sendCustomerRequestEmail(appointmentData, locals?.runtime?.env);
