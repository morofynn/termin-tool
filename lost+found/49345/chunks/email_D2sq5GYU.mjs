...
E-Mail: ${appointment.email}`,
    location: settings.standInfo || settings.companyAddress,
...
async function loadEmailSettings(env) {
  try {
    ...
    if (settingsJson) {
      ...
      return {
        companyName: settings.businessName || "MORO GmbH",
        companyAddress: settings.businessAddress || "Eupener Str. 124, 50933 Köln",
        companyPhone: settings.businessPhone || "+49 221 123456",
        companyEmail: settings.adminEmail || settings.businessEmail || "info@moro.de",
        companyWebsite: settings.businessWebsite,
        logoUrl: settings.logoUrl,
        primaryColor: settings.primaryColor || "#2d62ff",
        standInfo: settings.standInfo
      };
    }
  } catch (error) {
  ...
  return {
    companyName: "MORO GmbH",
    companyAddress: "Eupener Str. 124, 50933 Köln",
    companyPhone: "+49 221 123456",
    companyEmail: "info@moro.de",
    primaryColor: "#2d62ff"
  };
}
...
async function sendCustomerRequestEmail(appointment, env) {
  const settings = await loadEmailSettings(env);
  const html = generateCustomerRequestEmail(appointment, settings);
  return await sendEmail({
    to: appointment.email,
    subject: `⏳ Ihre Terminanfrage für ${formatDate(appointment.date)} um ${appointment.startTime}`,
    html,
    from: `${settings.companyName} <${settings.companyEmail}>`
  }, env);
}
async function sendCustomerConfirmationEmail(appointment, env) {
  const settings = await loadEmailSettings(env);
  const html = generateCustomerConfirmationEmail(appointment, settings);
  const ics = generateICS(appointment, settings);
  return await sendEmail({
    to: appointment.email,
    subject: `✅ Terminbestätigung: ${formatDate(appointment.date)} um ${appointment.startTime}`,
    html,
    icsAttachment: ics,
    from: `${settings.companyName} <${settings.companyEmail}>`
  }, env);
}
async function sendCustomerReminderEmail(appointment, env) {
  const settings = await loadEmailSettings(env);
  const html = generateCustomerReminderEmail(appointment, settings);
  return await sendEmail({
    to: appointment.email,
    subject: `⏰ Erinnerung: Ihr Termin morgen um ${appointment.startTime}`,
    html,
    from: `${settings.companyName} <${settings.companyEmail}>`
  }, env);
}
async function sendAdminNotification(appointment, action, env) {
  const settings = await loadEmailSettings(env);
  const html = generateAdminNotificationEmail(appointment, settings, action);
  let subject = "";
  switch (action) {
    case "requested":
      subject = `⏳ Neue Terminanfrage: ${appointment.name} am ${formatDate(appointment.date)} um ${appointment.startTime}`;
      break;
    case "confirmed":
      subject = `✅ Termin bestätigt: ${appointment.name} am ${formatDate(appointment.date)} um ${appointment.startTime}`;
      break;
    case "cancelled":
      subject = `❌ Termin storniert: ${appointment.name} am ${formatDate(appointment.date)} um ${appointment.startTime}`;
      break;
    case "rejected":
      subject = `❌ Termin abgelehnt: ${appointment.name} am ${formatDate(appointment.date)} um ${appointment.startTime}`;
      break;
  }
  return await sendEmail({
...
async function sendCustomerNotification(data, env) {
  ...
  switch (data.action) {
    case "requested":
      return await sendCustomerRequestEmail(appointment, env);
    case "instant-booked":
    case "confirmed":
      return await sendCustomerConfirmationEmail(appointment, env);
    case "cancelled":
    case "rejected":
      return await sendCustomerCancellationEmail(appointment, env);
    default:
...
async function sendReminderEmail(data, env) {
  const appointment = {
    ...
    date: data.day,
    startTime: data.time};
  return await sendCustomerReminderEmail(appointment, env);
}

export { sendCustomerNotification as a, sendReminderEmail as b, sendCustomerConfirmationEmail as c, sendCustomerRequestEmail as d, sendCustomerReminderEmail as e, sendAdminNotification as s, validateFormData as v };
